...
<body class="min-h-screen bg-slate-100 text-slate-800">
    <div class="flex min-h-screen items-center justify-center px-4 py-8">
        <div class="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            <h1 class="text-2xl font-black text-slate-900">Login Klub</h1>
            <p class="mt-1 text-sm text-slate-500">Masuk dengan Google untuk melanjutkan pendaftaran dan kelola data klub.</p>

            @include('partials.alerts')

            <div class="mt-6 space-y-3">
                @if ($googleConfigured)
                    <a href="{{ route('club.google.redirect') }}" class="inline-flex w-full items-center justify-center gap-3 rounded-xl border border-slate-300 px-4 py-3 text-sm font-bold text-slate-800 hover:bg-slate-50">
