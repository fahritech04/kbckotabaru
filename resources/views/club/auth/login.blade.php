...
@section('content')
    <div class="mx-auto w-full max-w-lg">
        <h1 class="text-3xl font-black text-slate-900">Login Klub</h1>
        <p class="mt-2 text-sm text-slate-500">Masuk dengan Google untuk melanjutkan pendaftaran dan kelola data klub.</p>

        <div class="mt-6 space-y-3">
            @if ($googleConfigured)
                <a href="{{ route('club.google.redirect') }}" class="inline-flex w-full items-center justify-center gap-3 rounded-xl border border-slate-300 px-4 py-3 text-sm font-bold text-slate-800 transition hover:-translate-y-0.5 hover:bg-slate-50">
            ...
            @else
                <div class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
                    Google Login belum aktif. Isi `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, dan `GOOGLE_REDIRECT_URI` di `.env`.
                </div>
            @endif
        </div>
